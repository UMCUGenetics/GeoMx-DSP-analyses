...
```{r addPlottingfactor}
add <-  function(overlay, annots, plottingFactor, countMatrix = "exprs"){
        ...
        #   print("A")
        # 
        #   #sampIDs <- which(apply(sData(as.data.frame(annots2)), 2, function(x){any(samples %in% x)}))                   
...
for (slide in unique(file$`slide name`)) {
  ...
  annots$ROILabel <- annots$roi
  annots$ROILabel <- gsub('"', '', annots$ROILabel)
  annots$ROILabel <- gsub('=', '', annots$ROILabel)
  ...
  assign(paste("img_", slide, sep = ""),createCoordFile(SpatialOverlay(slideName = slide,
...
#########PARAMETERS########
# Custom save order if list order deviates 
order <- c("hu_kidney_002","hu_kidney_003","hu_kidney_004","hu_kidney_001")
...
for (slide in slide_list) {
  counter = counter + 1
  #plot(plotSpatialOverlay(overlay = slide_list[[counter]], hiRes = FALSE, legend = FALSE, corner = "bottomright", 
  #                 scaleBarWidth = 0.5, textDistance = 5, scaleBarColor = "white",
  #                 fluorLegend = TRUE) + ggplot2::labs(title = paste("Whole slide OME-TIFF", slideName(slide))))
  
  # Get x and y limitations for florets
  xlim_1 <- max(res@phenoData@data$x[res@phenoData@data$slide == slideName(slide_list[[counter]])])
  xlim_2 <- min(res@phenoData@data$x[res@phenoData@data$slide == slideName(slide_list[[counter]])])
  ylim_1 <- max(res@phenoData@data$y[res@phenoData@data$slide == slideName(slide_list[[counter]])])
  ylim_2 <- min(res@phenoData@data$y[res@phenoData@data$slide == slideName(slide_list[[counter]])])
  
  # filter on slide
  ind <- pData(res)$slide_name == order[counter]
  res_ind <- res[,ind]
  ...
  fluor <- fluorLegend(overlay = slide_list[[counter]], nrow = 4, textSize = 15, alpha = 0.25)
  im <- plotSpatialOverlay(overlay = slide_list[[counter]], legend = FALSE, hiRes = FALSE, corner = "bottomright", 
                     scaleBarWidth = 0.5, textDistance = 5, scaleBarColor = "white", image = TRUE) + 
    ggplot2::labs(title = paste("Whole slide OME-TIFF", slideName(slide_list[[counter]])))
  ...
  legend("right",
         ...
         #legend = rownames(t(res_filter$beta)), cex = 1.5)
...
To see the specific genes or other areas you want to highlight, a
plotting factor has to be added.
Zoom into biological regions or regions under a certain condition with the param_zoom. Only available with conditions that include whole ROI's. You CANNOT zoom into what happens inside a ROI. Use the param_ANN to provide the ANN column in which contain the zoom values.
Color the gene-expression on the slide image with param_gene. You can also use an entire ANN column to color those values.

```{r}
#########PARAMETERS########
param_zoom <- "Proximal convoluted tubule"
param_ANN <- "ANN2"
param_gene <- "PODXL"
###########################
...
for (slide in slide_list){
  counter = counter + 1
  slide_list[[counter]] <- add(overlay = slide_list[[counter]], annots = target_Data, plottingFactor = param_gene)
  slide_list[[counter]] <- add(overlay = slide_list[[counter]], annots = target_Data, plottingFactor = param_ANN)
}

#img_overlay <- add(overlay = img_overlay, annots = target_Data, plottingFactor = "ANN1")
...
Here is an example of an image zoomed in on the ROIs "Proximal
convoluted tubule" and colored by the genexpression of "PODXL".

```{r plot zoomed slide image}
# Choose sample ids to zoom into
#samps <- target_Data@protocolData@data$SampleID[target_Data@phenoData@data$ANN2 == param_zoom]
samps <- target_Data@protocolData@data$SampleID[target_Data@phenoData@data %>% select(all_of(param_ANN)) == param_zoom ]

counter=0
# Crop the data around chosen sample ids
for (slide in slide_list){
  counter = counter + 1
  #assign(paste("Slide", counter, "Crop", sep = ""),cropSamples(overlay = slide_list[[counter]], sampleIDs = samps, sampsOnly = TRUE))
  assign(paste("Slide", counter, "Crop", sep = ""),cropSamples(overlay = slide_list[[counter]], sampleIDs = samps, sampsOnly = TRUE))
}
#img_overlayCrop <- cropSamples(overlay = slide_list[[1]], sampleIDs = samps, sampsOnly = TRUE)

counter=0
# Plot object with color gradient for gene expr data
for (slide in slide_list){
  counter = counter + 1
  plot(plotSpatialOverlay(overlay = get(paste("Slide", counter, "Crop", sep = "")), colorBy = param_gene, corner = "bottomright", 
                     scaleBarWidth = 0.5, textDistance = 5, scaleBarColor = "white"
                     ) + ggplot2::scale_fill_gradient2(low = "grey", high = "purple",  mid = "yellow") + ggplot2::labs(title = paste("Zoomed slide OME-TIFF", slideName(slide))))
}

# get(paste("Slide", counter, "Crop", sep = ""))
```
