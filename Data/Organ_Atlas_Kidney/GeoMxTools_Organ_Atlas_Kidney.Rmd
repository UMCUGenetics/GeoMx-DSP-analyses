...
datadir<-file.path("J:/1SURFDRIVE/USEQ/Nanostring/Pim/Data/organatlas_kidney/allyouneed/")
#datadir<-file.path("C:/Users/pkloosterman/Documents/general_workflow/organatlas_kidney")
#datadir<-file.path("L:/pkloosterman/Kidney_organ_atlas/")

#datadir<-file.path("C:/Users/pimkl/OneDrive/Documenten/UMCU/R/general_workflow/organatlas_kidney/")
datadir<-file.path("L:/pkloosterman/Kidney_organ_atlas/")
```
...
contrast_list: glomerulus = "Cortical glomerulus - Juxtamedullary
glomerulus" tub_glo = "Proximal convoluted tubule - Juxtamedullary
glomerulus" tubule = "Distal convoluted tubule - Proximal convoluted
tubule"

```{r contrast list}
# contrast_list <- list(
#   glomerulus = c("Cortical glomerulus", "Juxtamedullary glomerulus"),
#   tub_glo = c("Proximal convoluted tubule" ,"Juxtamedullary glomerulus"),
#   tubule = c("Distal convoluted tubule", "Proximal convoluted tubule")
# )

contrast_list <- list(
  glomerulus = c("Cor_Glo", "Jux_Glo"),
  tub_glo = c("Pro_Tub" ,"Jux_Glo"),
  tubule = c("Dis_Tub", "Pro_Tub")
)

pData(target_Data)[["ANN2_crop"]] <- gsub("Cortical glomerulus", "Cor_Glo", 
                                 gsub("Cortical filtration membrane", "Cor_Mem", 
                                 gsub("Juxtamedullary filtration membrane", "Jux_Mem",
                                 gsub("Juxtamedullary glomerulus", "Jux_Glo",
                                 gsub("Proximal convoluted tubule", "Pro_Tub",
                                 gsub("Distal convoluted tubule", "Dis_Tub",
                                 gsub("Collecting duct", "Col_Duc",
                                 gsub("Loop of Henle", "Henle",
                                 pData(target_Data)[["ANN2"]]))))))))
```

## t-test

```{r ttest, fig.width=30,fig.height=10 }
#gc()
plots<-list()
tables<-list()
labels<-list()
...
for (contrast in names(contrast_list)) {
  ...
  active_group2 <- contrast_list[contrast][[1]][[2]]
      ...
      group1<-log_q[,names(as.data.frame(assayDataElement(target_Data, elt= "exprs")))[pData(target_Data)$ANN2_crop==active_group1]]
      group2<-log_q[,names(as.data.frame(assayDataElement(target_Data, elt= "exprs")))[pData(target_Data)$ANN2_crop==active_group2]]
      ...
      plots[[counter]]<- ggplot(results,
        ...
        theme(legend.position = "bottom") +
        ggtitle(paste("class: ", contrast," - ",test, mtc,"multitest corr"))
...
pData(target_Data)$testStatus <- factor(pData(target_Data)$ANN2_crop, unique(target_Data$ANN2_crop))
...
for (contrast in names(contrast_list)) {
    active_group1 <- contrast_list[contrast][[1]][[1]]
    active_group2 <- contrast_list[contrast][[1]][[2]]
    ind <- pData(target_Data)$ANN2_crop == active_group1 | pData(target_Data)$ANN2_crop == active_group2

    mixedOutmc <-
        mixedModelDE(target_Data[,ind],
                     elt = "log_q",
                     modelFormula = ~ testStatus + (1 | slide),
                     groupVar = "testStatus",
...
```{r lmm_vulcano, fig.width=30,fig.height=15}
...
for(c in unique(lmm_results$Contrast) ) {
  ...
  top_g <- c(top_g,
    lmm_results_slice[, 'Gene'][
      order(lmm_results_slice[, 'invert_P'], decreasing = TRUE)[1:30]],
    lmm_results_slice[, 'Gene'][
      order(lmm_results_slice[, 'invert_P'], decreasing = FALSE)[1:30]])
...
grid.arrange(grobs=plots_lmm,ncol=3)
```    
...
my_gois <-unique(subset(lmm_results, `FDR` < 0.001)$Gene)[1:100]    # 100 to prevent long runtime
...
# top_all_subset <- top_all[top_all$Contrast == "Pro_Tub - Dis_Tub"]

# ggplot(top_all_subset,aes(x=reorder(pathway, NES),y=NES,fill=Color)) + 
#   scale_x_discrete(labels = function(x) str_wrap(x, width = 100)) + 
#   geom_col(position="dodge") + scale_fill_manual(values = c("#2ECC71", "#E74C3C")) +
#   ggtitle("Top 15 + and - NES reactome pathways") + 
#   xlab("pathway") + ylab("Normalized Enrichment Score") +
#   coord_flip() + 
#   theme(text = element_text(size = 20)) +
#   theme(panel.background = element_blank())
...
```{r Add coords}
#Get coordinates into the meta data. Mostly for spatial accurate deconvolution in 10.2

#xy <- read_excel("L:/pkloosterman/Kidney_organ_atlas/annotation/OrganAtlas_Kidney.xlsx", sheet = "SegmentProperties")
xy <- read_excel(SampleAnnotationFile, sheet = "SegmentProperties")
xy <- xy[,c("SegmentDisplayName", "ROICoordinateX", "ROICoordinateY")]
...
annot$x <- annot$ROICoordinateX
annot$y <- annot$ROICoordinateY
target_Data@phenoData@data["x"] <- annot$x
target_Data@phenoData@data["y"] <- annot$y
```
...
pData(target_Data)$region <- 
    factor(pData(target_Data)$ANN2, unique(target_Data$ANN2))   # Must be manual?
variables_to_plot <- c("slide_name","ANN2")                     # Must be manual?
...
# define variables to show in heatmaps:
variables_to_plot <- c("slide_name", "ANN2")
...
# 10 Slide image reference

The SpatialOmicsOverlay package shows where the placement of the ROIs
are within the slide image. It is possible to zoom into specific ROIs
with a certain region or status and see the gene expression or a gene
signature between the ROIs.

# 10.1 Set up adjusted functions

Three original functions have been adjusted to accept client data.

addPlottingFactor has a built in check to see if the sample IDs completely match. To do this, the function grabs sample IDs from the GeoMx DSP object with sData() and making it a matrix.
This causes an error because sData(target_data) returns an object that has the wrong dimensions for as.matrix to work. 
Due to this part being optional it can be skipped to make this function work.

parseOverlayAttrs and annotMatching work under the impression that the ROI labels are numeric (001, 002, etc). With the client data, this isnt the case.
Luckily, this seems to be an overcorrection for specific nanoString data and the numeric step can be skipped.

```{r addPlottingfactor}
addPlottingFactor2 <-  function(overlay, annots, plottingFactor, countMatrix = "exprs"){
...
```{r annotMatching}
annotMatching2 <- function(annots, ROInum, maskNum, maskText){
...
```{r parseOverlayAttrs}
parseOverlayAttrs2 <- function(omexml, annots, labworksheet){
    ...
    for(ROI in names(ROIs)){
        ...
        for(mask in masks){
            ...
            ROIannot <- annotMatching2(annots, ROInum, maskNum, maskText)
...
# 10.2 Load SpatialOverlay Object

Due to the requirement of adjusting functions, the SpatialOveray object has to be created by calling a few functions instead of calling the main function.

```{r Load SpatialOverlay object, echo=FALSE}
# Load files. Select the right sheet.
file <- read_excel(SampleAnnotationFile, sheet = "trimmed_set")
slides <- unique(file$`slide name`[!file$`slide name` == "No Template Control"])

slide_list <- c()
for (slide in unique(file$`slide name`)) {
  tifFile <- paste(datadir,"/OMETIFF/", slide, ".ome.tiff", sep = "")
  print(tifFile)
  
  # Check if OMETIFF files exist, skip if not.
  if (!file.exists(tifFile)){next}
  
  # Grab info from the right slide
  annots <- file[file$`slide name` == slide,]
  annots$ROILabel <- gsub('"', '',
                     gsub('=', '', 
                     annots$roi))
  
  # patialOmicsOverlay funtions
  xml <- xmlExtraction(ometiff = tifFile)
  AOIattrs <- parseOverlayAttrs2(omexml = xml, annots = annots, labworksheet = TRUE)
  scan_metadata <- parseScanMetadata(omexml = xml)
  
  # Correcting values to make sure it gets passed.
  ...
  # Assign variables to SpatialOmicsOverlay objects
  assign(paste("img_", slide, sep = ""),createCoordFile(SpatialOverlay(slideName = slide, scanMetadata = scan_metadata,
                       overlayData = AOIattrs, workflow = list(outline=outline, scaled=FALSE)), outline = FALSE))
  
  assign(paste("img_", slide, sep = ""),addImageOmeTiff(overlay = get(paste("img_", slide, sep = "")), ometiff = tifFile, res=6))
  
  slide_list <- c(slide_list, get(paste("img_", slide, sep = "")))
}
slide_list <- setNames(slide_list, slides)
```

The whole slide images with the ROIs highlighted and its Spatial Deconvolution. 
The slides are added to the list as they are ordered in the folder. If this deviates from the logical order, adjust it with the parameters so it is correctly linked).
If you are looking for the spatial deconvolution of specific cells, add those cells into the cell_param parameter.

```{r Plot whole slide image, fig.width=25,fig.height=30, out.width="50%"}
#########PARAMETERS########
# Choose deconvolution groups
#cell_param <- c(colnames(res$beta))  # all groups
cell_param <- c("Neutrophil", "Fibroblast", "NK.cell", "B.cell")
###########################

# Create colors for deconvolution
P36 = createPalette(length(cell_param),  c("#ff0000", "#00ff00", "#0000ff"))

# Plot object with deconvolution
for (slide in slide_list) {
  # Get x and y limitations for florets
  xlim_1 <- max(res@phenoData@data$x[res@phenoData@data$slide == slideName(slide)])
  xlim_2 <- min(res@phenoData@data$x[res@phenoData@data$slide == slideName(slide)])
  ylim_1 <- max(res@phenoData@data$y[res@phenoData@data$slide == slideName(slide)])
  ylim_2 <- min(res@phenoData@data$y[res@phenoData@data$slide == slideName(slide)])
  
  # filter on slide
  ind <- pData(res)$slide_name == slideName(slide)
  res_ind <- res[,ind]
  res_filter <- res_ind[!res_ind@phenoData@data[["beta"]]==0]
  
  # filter on cell choice
  df <- as.data.frame(res_filter$beta)
  values <- df[cell_param]
  
  # Flip y coords to match deconvolution
  #slide_list[[counter]] <- flipY(slide_list[[counter]])  # Command out when rerunning
  
  # Save the image and its legend and draw it with ggdraw for better formatting
  fluor <- fluorLegend(overlay = slide, nrow = 4, textSize = 15, alpha = 0.25)
  im <- plotSpatialOverlay(overlay = slide, legend = FALSE, hiRes = FALSE, corner = "bottomright", 
                     scaleBarWidth = 0.5, textDistance = 5, scaleBarColor = "white", image = TRUE) + 
    ggplot2::labs(title = paste("Whole slide OME-TIFF", slideName(slide)))
  
  print(cowplot::ggdraw() +
    cowplot::draw_plot(im, x = 0, y = 0) +
    cowplot::draw_plot(fluor, scale = 0.15, x = 0.45, y = 0)) #+ #, scale = 0.12, x = -0.3, y = -0.25)
  
  # Plot florets on empty plot to adjust its x axis
  plot(0, xlim = c(xlim_2, xlim_1+5000), ylim = c(ylim_1+1000, ylim_2-1000), axes = FALSE, xlab = "", ylab = "") # +5000 to make room for the legend, +/-1000 is to make room for the horizontal line to section of plots
  
  florets(
    x = res_ind@phenoData@data$x,# + 40000,
    y = res_ind@phenoData@data$y,
    col = P36,
    #xlim = c(0,xlim+40000),
    #ylim = c(0,ylim),
    b = t(values),
  ...
  legend("right",
         fill = P36,
         legend = rownames(t(values)), cex = 3)
 
  abline(h=ylim_2-1000, col = "black")
  abline(h=ylim_1+1000, col = "black")
}
```

# 10.3 Add plotting factors

To see the specific genes or other areas you want to highlight, a plotting factor has to be added.

Zoom into biological regions or regions under a certain condition with the param_zoom_value. Only available with conditions that include whole ROI's. You CANNOT zoom into what happens inside a ROI. Use the param_ANN to provide the ANN column that contains the zoom values.
Color the gene-expression on the slide image with param_gene. You can also use an entire ANN column to color those values.

```{r}
#########PARAMETERS########
param_zoom_value <- "Proximal convoluted tubule"
param_ANN <- "ANN2"
param_gene <- "PODXL"
###########################
...
for (slide in slide_list){
  slide_list[[slideName(slide)]] <- addPlottingFactor2(overlay = slide, annots = target_Data, plottingFactor = param_gene)
  slide_list[[slideName(slide)]] <- addPlottingFactor2(overlay = slide, annots = target_Data, plottingFactor = param_ANN)
}

# Choose sample ids to zoom into. Grabs sample IDs that fall under the param_zoom_value condition.
samps <- target_Data@protocolData@data$SampleID[target_Data@phenoData@data %>% select(all_of(param_ANN)) == param_zoom_value]

# Show plotting factors as table
#plotFactors(slide_list[[1]])

# Get mean for midpoint reference
#mean(img_overlay@plottingFactors[["UBD"]])
```

# 10.4 Plot zoomed slide image

cropSamples automatically crops the image around the ROIs given. Other ROIs in the cropped image can be kept in or ignored. Below we will crop to only ROIs that are given, hiding ROIs profiled which are not. Setting sampsOnly = TRUE hides the non-given ROIs which are within the plotted region.

```{r plot zoomed slide image}
# Crop the data around chosen sample ids
#counter=0
for (slide in slide_list){
  #counter = counter + 1
  assign(paste(slideName(slide), "Crop", sep = ""),cropSamples(overlay = slide, sampleIDs = samps, sampsOnly = TRUE))

  plot(plotSpatialOverlay(
    overlay = get(paste(slideName(slide), "Crop", sep = "")), 
    colorBy = param_gene, 
    corner = "bottomright", 
    scaleBarWidth = 0.5, 
    textDistance = 5, 
    scaleBarColor = "white") + 
      ggplot2::scale_fill_gradient2(low = "grey", high = "purple",  mid = "yellow") + 
      ggplot2::labs(title = paste("Zoomed slide OME-TIFF", slideName(slide))))
}
```

```{r Plot whole slide image, fig.width=10,fig.height=10, out.width="50%"}
#########PARAMETERS########
# Choose deconvolution groups
cell_param <- c(colnames(res$beta))  # all groups
#cell_param <- c("Neutrophil", "Fibroblast", "NK.cell", "B.cell")
...
for (slide in slide_list) {
  ...
  # Get x and y limitations for florets
  x1 <- max(get(paste(slideName(slide), "Crop", sep = ""))@overlayData@position[["X"]])
  x2 <- min(get(paste(slideName(slide), "Crop", sep = ""))@overlayData@position[["X"]])
  y1 <- max(get(paste(slideName(slide), "Crop", sep = ""))@overlayData@position[["Y"]])
  y2 <- min(get(paste(slideName(slide), "Crop", sep = ""))@overlayData@position[["Y"]])
  ...
  # Save the image and its legend and draw it with ggdraw for better formatting
  fluor <- fluorLegend(overlay = get(paste(slideName(slide), "Crop", sep = "")), nrow = 4, textSize = 5, alpha = 0.25)
  im <- plotSpatialOverlay(overlay = get(paste(slideName(slide), "Crop", sep = "")), legend = FALSE, hiRes = FALSE, corner = "bottomright", 
                     scaleBarWidth = 0.5, textDistance = 5, scaleBarColor = "white", image = TRUE) + 
    ggplot2::labs(title = paste("Whole slide OME-TIFF", paste(slideName(slide), "Crop", sep = "")))
  
  print(cowplot::ggdraw() +
    cowplot::draw_plot(im, x = 0, y = 0) +
    cowplot::draw_plot(fluor, scale = 0.15, x = 0.45, y = 0)) #+ #, scale = 0.12, x = -0.3, y = -0.25)
  
  # Prepare plot space for florets including legend
  opar <- par(no.readonly = TRUE)
  par(mar = c(5, 5, 4, 20))
  
  florets(
    x = get(paste(slideName(slide), "Crop", sep = ""))@overlayData@position[["X"]],
    y = get(paste(slideName(slide), "Crop", sep = ""))@overlayData@position[["Y"]],
    col = P36,
    xlim = c(x2,x1),
    ylim = c(y1,y2),
    b = t(values),
