...
ggplot(test_gr, aes(x, id = id, split = y, value = n)) +
  geom_parallel_sets(aes(fill = ANN2), alpha = 0.5, axis.width = 0.4) +
  geom_parallel_sets_axes(axis.width = 0.4) +
  geom_parallel_sets_labels(color = "white", size = 5, angle = 0, ) +
  ...
  scale_fill_manual(values=color_list$ANN2) +
  labs(x = "", y = "") +
  annotate(geom = "segment", x = 3.45, xend = 3.45, y = 10, 
           yend = 60, lwd = 2) +
  annotate(geom = "text", x = 3.39, y = 25, angle = 90, size = 5,
           hjust = 0.5, label = "50 segments")
```
...
# 5.1 Visualize the first 8 segments with each normalization method {.tabset .tabset-pills}
...
```{r clustering_subset, fig.width=10,fig.height=15, eval=subset_needed}
# create a log2 transform of the data for analysis
assayDataElement(object = target_Data, elt = "log_q") <-
  assayDataApply(target_Data, 2, FUN = log, base = 2, elt = "q_norm")
...
clust_method <- "average"    # options: average, ward.D, ward.D2, single, complete, mcquitty, median and centroid.
# choose clustering distance methods for heatmaps
dist_method <- "euclidean" # options: correlation, euclidean, maximum, manhattan, canberra, binary and minkowski.
...
# Choose manually or it picks the highest significant pathway
pathway_name <- "INSULIN SIGNALING PATHWAY" # manual pathway query. "" for non manual entry
...
ANN <- "ANN2" # ANN column with conditions
